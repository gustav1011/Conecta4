...
int colunaMaquina;
do {
    colunaMaquina = rand() % 7; // Escolhe uma coluna aleatória
} while (getColunaDisponivel(colunaMaquina) == -1); // Garante que a coluna não está cheia
...
if (colunaMaquina >= 0 && colunaMaquina <= 7) {
    ...
    int linhaDisponivelMaquina = getColunaDisponivel(colunaMaquina);
    if (linhaDisponivelMaquina != -1) {
        tabuleiroAux[linhaDisponivelMaquina][colunaMaquina] = jogadorAtual; // Atualiza o tabuleiro
        ...
        jogadorAtual = (jogadorAtual == 1) ? 2 : 1; // Alterna para o jogador
    } else {
...
void jogadaMaquina(SDL_Renderer* renderer, SDL_Texture* tabuleiro,  SDL_Texture* ficha_amarela,SDL_Texture* ficha_transparente, SDL_Rect quad1) {
    ...
    //seleciona_ficha = 2; // Zera a seleção da ficha
    ...
    while (running && !fichaCaiu) {
        while (SDL_PollEvent(&event)) { // Processa eventos
            ...
            mouse_x>100 && mouse_x<232 && mouse_y>400 && mouse_y<532;
              seleciona_ficha = 2; // Seleciona a ficha amarela
                     // Seleciona a ficha amarela
